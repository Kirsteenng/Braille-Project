...
class BrailleClassifier(pl.LightningModule):
    ...
    def train_dataloader(self):
        train_data = Braille_Dataset(path_data=self.dataset_path, resize=True, mode='train', debug=debug)
    ...
    def val_dataloader(self):
        val_data = Braille_Dataset(path_data=self.dataset_path, resize=True, mode='val', debug=debug)
...
def run_trainer():
    ...
    logger = TensorBoardLogger('tb_logs2', name='braille_model_depth_v1')
    if not with_cuda:
    ...
    trainer.fit(model)
    print("Saving  model ... ")
    trainer.save_checkpoint(f"./depth/no_aug/braille_model_{hparams['model_type']}.ckpt")
    print("Saved")
