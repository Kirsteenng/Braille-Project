...
class Braille_Dataset(Dataset):
    def __init__(self, path_data='./data', gray_depth=False, resize=False, mode="train", transformer=None, debug=False): #mode=['train', 'val', 'test']
        self.rgb_dataset = []
        self.depth_dataset = []
        self.target_dataset= []
        self.w = 367
        ...
        for l in range(len(letters)):
            ...
            if(mode=='train'):
                ...
                self.target_dataset = self.target_dataset + [np.array([labels[l]])]*(num_img_val-num_img_train)
            elif(mode=='test'):
                num_img_train = int(num_img*0.7)
                num_img_test = num_img_train + int(num_img*0.2)
                tmp = list(rgb_imgs[num_img_test:])
                self.rgb_dataset = self.rgb_dataset + tmp
                self.depth_dataset = self.depth_dataset + list(depth_imgs[num_img_test:])
                self.target_dataset = self.target_dataset + [np.array([labels[l]])]*len(tmp)
            else:
    ...
    def shuffle_dataset(self):
        idx = np.random.randint(len(self.target_dataset), size=len(self.target_dataset))
        self.rgb_dataset = np.array(self.rgb_dataset)[idx]
        self.depth_dataset = np.array(self.depth_dataset)[idx]
        self.target_dataset = np.array(self.target_dataset)[idx]
